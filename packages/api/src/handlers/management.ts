...
async function handleTenantOperations(
  ...
  switch (method) {
    ...
    case 'PUT': {
      ...
      const { error: overrideError, value: overrideValue } =
        validateTenantOverrideRequest(overrideBody);
...
async function handleEmergencyOperations(
  ...
  switch (method) {
    case 'POST': {
      ...
      await client.setKillSwitch(flagKey || null, true, reason, activatedBy);
